    ...
    @foreach ($items as $key => $value)
        <div class="form-group col-md-6" id="{{ $id = sprintf('%s-list-%s', $attribute, $key) }}">
            <div class="input-group">
                <input name="{{ sprintf('%s[%s]', $attribute, $key) }}" type="text" value="{{ $value }}" class="form-control {{ sprintf('ta-%s', $attribute) }} {{ $errors->{$errorBag ?? 'default'}->has(sprintf('%s.%s', $attribute, $key)) ? 'is-invalid' : '' }}" maxlength="255" placeholder="@lang ('Please select')" autocomplete="off" oninput="window.Common.checkInvalidMaster('{{ $attribute }}', 'invalid-feedback-{{ sprintf('%s.%s', $attribute, $key) }}', this.value);" />
