    ...
    <div class="container">
        <div class="row justify-content-center">
            <div class="col-md-5">
                ...
                <form action="{{ route('register') }}" method="POST" autocomplete="off" onsubmit="window.Common.overlay();">
                    {{ csrf_field() }}
                    ...
                    <div class="input-prepend input-group">
                        ...
                        @set ($attribute, 'name')
                        <input name="{{ $attribute }}" type="text" value="{{ $errors->{$errorBag ?? 'default'}->any() ? old($attribute) : null }}" class="form-control {{ $errors->{$errorBag ?? 'default'}->has($attribute) ? 'is-invalid' : '' }}" maxlength="255" placeholder="@lang ('Name')" required autofocus />
                    ...
                    <div class="input-prepend input-group mt-1">
                        ...
                        @set ($attribute, 'email')
                        <input name="{{ $attribute }}" type="email" value="{{ $errors->{$errorBag ?? 'default'}->any() ? old($attribute) : null }}" class="form-control {{ $errors->{$errorBag ?? 'default'}->has($attribute) ? 'is-invalid' : '' }}" maxlength="255" placeholder="@lang ('E-Mail')" required />
                        @component ('components.messages.invalid', ['name' => $attribute]) @endcomponent
                    </div>
